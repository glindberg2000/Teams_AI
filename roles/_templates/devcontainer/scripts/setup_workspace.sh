...
# 6. Clone internal_chat_mcp if not present
if [ ! -d "/workspaces/project/internal_chat_mcp" ]; then
  log "Cloning internal_chat_mcp repo..."
  if ! git clone https://github.com/glindberg2000/internal_chat_mcp.git /workspaces/project/internal_chat_mcp; then
    log "WARNING: Failed to clone internal_chat_mcp repo, continuing..."
  fi
fi

# 7. Install internal_chat_mcp in the container venv
if [ -d "/workspaces/project/internal_chat_mcp" ]; then
  ...
  log "INFO: internal_chat_mcp repo not found, skipping install."
fi

# 8. Clone main project repo if not present and not a placeholder
