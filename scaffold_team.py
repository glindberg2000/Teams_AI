...
def generate_env_file(project, prefix, domain, roles, dry_run=False):
    """
    Generate the environment file (.env.{project}) with all required configuration.
...
def generate_env_template(project, roles, dry_run=False):
    ...
    actual values, stored in teams/{project}/env.template.
    ...
    project_dir = Path(f"teams/{project}")
...
def generate_checklist(project, roles, dry_run=False):
    ...
    Creates a markdown file with setup instructions in teams/{project}/checklist.md.
    ...
    content = [
        ...
        f"- [ ] Create a `.env.{project}` file in the root (already done - see details below)",
        f"- [ ] Fill in all required API keys in `.env.{project}`",
        f"- [ ] Run `python team-cli/team_cli.py create-crew --env-file .env.{project}` to create all sessions",
        "",
        "## API Keys Required",
        "",
        "### Team Level",
    ...
    content.extend(
        [
            ...
            f"- Use `team-cli/team_cli.py create-crew --env-file .env.{project}` to create all sessions at once",
...
def main():
    ...
    try:
        ...
        print(f"2. Run 'python team-cli/team_cli.py create-crew --env-file {env_file}'")
        print(
            f"3. Follow the checklist at {checklist if not args.dry_run else 'teams/' + project + '/checklist.md'}"
        )
